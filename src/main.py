...
def Ratio_11_solvabilite(Code=Query(...,description="Code APE de l'entreprise."),
                         TB=Query(...,description="Total Bilan à insérer, code EE sur liasse fiscale  2050."),
                         FP=Query(...,description="Fonds Propres à insérer, code DL sur liasse fiscale 2050.")):
    return(BACH_R11(Code,TB,FP,table_BACH))

@app.get("/R28")
def Ratio_28(Code=Query(...,description="Code APE de l'entreprise."),
             DU=Query(...,description="emprunts et dettes auprès desetablissements de crédit, code DU sur liasse fiscale 2050."),
